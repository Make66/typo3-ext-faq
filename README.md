# FAQ Extension for TYPO3 (Taketool/Faq)

## User Story

As a **TYPO3 editor**, I want to create FAQ entries and organize them into categories so that website visitors can browse frequently asked questions in a clean, collapsible accordion — grouped by topic, with each category and question expandable on click.

I select one or more storage pages and a parent category in the plugin's FlexForm. The extension then displays all FAQ records from those pages whose categories are children of the selected parent, rendered as a nested Bootstrap 5 accordion — outer level per category, inner level per question — all collapsed by default.
